...
export default function UserReports() {
  ...
  // Define hover background once in the component body
  const hoverBg = useColorModeValue("#edf2f7", "#2D3748");
...
{data && data.length > 0 && (
  <Card>
    ...
    {!isLoding ? (
      <>
        {(() => {
          const maxValue = Math.max(...data.map((item) => item.length));
          return data.map((item, i) => {
            const barWidth = maxValue > 0 ? (item.length / maxValue) * 100 : 0;
            return (
              <Box
                key={item?.name || i}
                p={2}
                ...
                <GridItem colSpan={{ base: 12, md: 6 }}>
                  ...
                  >
                    ...
                    <Text fontWeight={600} color="#d6401d">
                      <CountUpComponent
                        targetNumber={findLeadStatus("sold")}
                      />
                    </Text>
                  </Box>
                </GridItem>
